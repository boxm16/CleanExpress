...
begin
//πρωτα naxodim poslednii Batch(naxozhu standartnoe slovo (Deleted :) v konce i vichisljaju mesto poslednego Batch
  nRowDB:=DBForm.DB.Cols[0].IndexOf('Deleted :');
 	if (nRowDB > -1) then begin
  nRowDB:=nRowDB-8;
  EditLab1.Text:=DBForm.DB.Cells[1,nRowDB];
end;
//τωρα ψαχνω αν έχει το ΕΠΟΜΕΝΟ BATCH  στο excel που εισαγω
...
else ShowMessage('Δεν μπορώ να βρω το τελευταιο Batch στο Excel, βαλε άλλο Excel');

 //now i copy Αυξων αριθμος culomn στο τελευταίο culomn, για να μπορεσω μετα να κανω SynchroPoziTron.
     ...
     DBForm.DB.Row:=nRow;
      ...
      StartRow:=nRow;
       ...
       if startHour<StartHourStopSignal then begin //zdes zadaju  stop signal esli izmenjaetsja value o hour
                                //nu, αφου βρήκαμε, μπορευμε και να κάτσουμε
  StartRow:=nRow;
       exit;
    end;
...
KodikosEidous:=MainForm.EditLab1.SelText;
  //εδώ υπολογίζω τα μέτρα για κουρτίνεσ  βάσει τις συνολικής αξίας δια τιμή, ταύτόχρονα στρογγυλοποιόντας το αποτέλεσμα με SimpleRoundTo (dlja etoi funkcii nuzhno podkoluchit modul Math in uses
...
label 10;
begin
//πρωτα αδειαζω  PavlosBasket, MparakosBasket and MpoyrmpakisBasket and AkirosisBasket  απο προηγουμενη παρτιδα
...
FindEndTime.Click;
 ShowBasketForm.Click;
   ...
   i:=StartRow;
10:
//zdesvideljaju κωδικό είδους από περιγραφή είδους
...
//τωρα πρεπει να βρουμε στο τιμοκαταλογο τον ζητούμενο κωδικο
...
//vivozhu teksti
PAVLOS_DA_TIM.Cells[2,3]:='Σύνολο';
PAVLOS_DA_TIM.Cells[2,4]:='Έκπτωση';
  //εδω βγαζω εκπτωσεις για μαγιαζια
  if MainForm.Caption='KALLITHEA' then
PAVLOS_DA_TIM.Cells[3,4]:='0'  else
PAVLOS_DA_TIM.Cells[3,4]:='5';

PAVLOS_DA_TIM.Cells[2,5]:='Σύνολο με έκπτωση';
PAVLOS_DA_TIM.Cells[2,6]:='ΦΠΑ';
PAVLOS_DA_TIM.Cells[3,6]:='24';
PAVLOS_DA_TIM.Cells[2,7]:='ΤΕΛΙΚΟ ΣΥΝΟΛΟ';
...
begin
for a:=8 to PAVLOS_DA_TIM.RowCount-1 do
//πρωτα υπλογίζω αξία του κάθε είδους ξεχωριστά
begin
...
var
  ...
  I, K: Integer;
begin
 SynchroPoziTron.Execute;//πρωτα κανω έλεγχο αν εχει γινει αποσυγχρονισμος
...
if   DBForm.DB.Cells[13,b]<>'' then  begin
 ShowMessage('ΤΟ ΤΕΜΑΧΙΟ'+BasketForm.PavlosBasket.Text+ 'ΕΧΕΙ ΣΤΑΛΕΙ ΗΔΗ ΜΕ ΔΕΛΤΙΟ ΑΠΟΣΤΟΛΗΣ-'+DBForm.DB.Cells[13,b]);
...
{//vivozhu teksti
MPARAKOS_DA_TIM.Cells[2,3]:='Σύνολο';
MPARAKOS_DA_TIM.Cells[2,4]:='Έκπτωση';
  //εδω βγαζω εκπτωσεις για μαγιαζια
  if MainForm.Caption='KALLITHEA' then
MPARAKOS_DA_TIM.Cells[3,4]:='0'  else
MPARAKOS_DA_TIM.Cells[3,4]:='5';

MPARAKOS_DA_TIM.Cells[2,5]:='Σύνολο με έκπτωση';
MPARAKOS_DA_TIM.Cells[2,6]:='ΦΠΑ';
MPARAKOS_DA_TIM.Cells[3,6]:='24';
MPARAKOS_DA_TIM.Cells[2,7]:='ΤΕΛΙΚΟ ΣΥΝΟΛΟ';}
...
a:integer;
  ...
  RowCount, ColCount, i, j: Integer;
begin
//πρωτα PAVLOS_DA_TIM_INVS-shi valageb ise rogorc minda ro gamovagdo mere excel-shi
...
begin
a:=D_DeltioApostolis.RowCount;
//zdes dobavljaju NO στο τελος του δελτιου, για να το εχω οταν φορτονω
...
//sdes dobavljaju IX_Number  στο τελος του δελτιου, για να το εχω οταν φορτονω
D_DeltioApostolis.Cells[3,a]:=IX_Number.Text;

// τωρα perenoshu DeltioApostolis v Archive
...
//sdes dobavljaju arithmo deltiou apostolis v DB
 ...
 b:=DBForm.DB.Cols[2].IndexOf(BasketForm.MpoyrmpakisBasket.Text);
 if   DBForm.DB.Cells[13,b]<>'' then  begin
 ShowMessage('ΤΟ ΤΕΜΑΧΙΟ'+BasketForm.MpoyrmpakisBasket.Text+ 'ΕΧΕΙ ΣΤΑΛΕΙ ΗΔΗ ΜΕ ΔΕΛΤΙΟ ΑΠΟΣΤΟΛΗΣ-'+DBForm.DB.Cells[13,b]);
...
D_TimologisiForm.D_Timologio.Cells[0,2]:='Κωδικός';
D_TimologisiForm.D_Timologio.Cells[1,2]:='Στείλαμε';
D_TimologisiForm.D_Timologio.Cells[2,2]:='Παραλάβαμε';
D_TimologisiForm.D_Timologio.Cells[3,2]:='Τιμή';
D_TimologisiForm.D_Timologio.Cells[4,2]:='ΦΠΑ';
D_TimologisiForm.D_Timologio.Cells[5,2]:='Τιμή με ΦΠΑ';
D_TimologisiForm.D_Timologio.Cells[6,2]:='Τιμή CLEAN POINT';
D_TimologisiForm.D_Timologio.Cells[6,1]:='ΣΥΝΟΛΟ';
end;
...
PK_Archive.Cells[0,a]:=PK_DeltioApostolis.Cells[1,0];
                            //zapolnjau αριθμο Δελτιου Αποστολης, μαζι με τις ημερομηνιες και ωρες
 PK_Archive.Cells[1,a]:=PK_DeltioApostolis.Cells[1,1]+'('+
   ...
   for c:=3 to PK_DeltioApostolis.RowCount-1 do begin
    d:=PK_Archive.Rows[0].IndexOf(PK_DeltioApostolis.Cells[0,c]);
       if d=-1 then begin   //αν δεν εχω τετοιο κωδικο ακομα, πρωτα βαζω στη λιστα προιοντων, και μετα βαζω και τεμαχιο
       ...
       else    //αφου υπαρχει τετοιος κωδικος στη λιστα προινοντων, απλα βαζω τεμάχια
...
//sdes dobavljaju arithmo deltiou apostolis v DB
 ...
 b:=DBForm.DB.Cols[2].IndexOf(BasketForm.MparakosBasket.Text);
 if   DBForm.DB.Cells[13,b]<>'' then  begin
 ShowMessage('ΤΟ ΤΕΜΑΧΙΟ'+BasketForm.MparakosBasket.Text+ 'ΕΧΕΙ ΣΤΑΛΕΙ ΗΔΗ ΜΕ ΔΕΛΤΙΟ ΑΠΟΣΤΟΛΗΣ-'+DBForm.DB.Cells[13,b]);
...
a:integer;
begin
//sperva zapolnjaju MparakosBasket με τεμάχια όλου του Δελτιου Αποστολής (PackageDeal)
...
begin
       ...
       // τωρα perenoshu B_DeltioApostolis v Archive
...
B_Archive.Cells[0,a]:=B_DeltioApostolis.Cells[1,0];
                            //zapolnjau αριθμο Δελτιου Αποστολης
 B_Archive.Cells[1,a]:=B_DeltioApostolis.Cells[1,1];
   ...
   for c:=3 to B_DeltioApostolis.RowCount-1 do begin
    d:=B_Archive.Rows[0].IndexOf(B_DeltioApostolis.Cells[0,c]);
       if d=-1 then begin   //αν δεν εχω τετοιο κωδικο ακομα, πρωτα βαζω στη λιστα προιοντων, ενδεικτική τιμη προιοντος(1) και μετα βαζω και τεμαχιο
...
//sdes dobavljaju arithmo deltiou apostolis v DB
 ...
 ShowMessage('ΤΟ ΤΕΜΑΧΙΟ-'+BasketForm.MparakosBasket.Text+ '-ΕΧΕΙ ΕΡΘΕΙ ΗΔΗ ΜΕ ΔΕΛΤΙΟ ΑΠΟΣΤΟΛΗΣ-'+DBForm.DB.Cells[13,b]);
...
//σβηνω αμεσως κατι που δεν χρειαζομε, το ειπα και πανω οτι θα σβησω
